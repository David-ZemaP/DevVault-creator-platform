...
describe('Marketplace routes', () => {
  ...
  beforeEach(() => {
    ...
    const selectChain: any = { eq: () => selectChain, maybeSingle: async () => ({ data: { project_id: project.id }, error: null }) };
    dbModule.commerceDb = () => ({ from: () => ({ select: () => selectChain, insert: async (row: any) => { if (records.some(r => r.transaction_hash === row.transaction_hash)) return { error: { code: '23505' } }; records.push(row); return { error: null }; }, update: () => updateChain }) });
