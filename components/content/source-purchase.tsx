  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <p className="text-2xl font-bold text-white">{priceEth} HSK</p>
        <p className="text-sm text-neutral-400">
          {isSubscription
            ? '30-day subscription · Access to code & updates while subscribed'
            : 'One-time purchase · permanent access'}
        </p>
        <p className="text-xs text-neutral-500">HSKChain Testnet (chain 133)</p>
      </div>

      {state === 'creator' ? (
        <p className="text-sm text-neutral-400">
          Your project · <Link href={`/create?id=${id}`} className="text-red-400">Manage project</Link>
        </p>
      ) : isSubscription ? (
        access.hasActiveMembership ? (
          <p className="text-emerald-400 text-sm font-medium">Subscribed · Active ✓</p>
        ) : access.purchased ? (
          <p className="text-amber-400 text-sm font-medium">Subscription Expired</p>
        ) : null
      ) : state === 'purchased' ? (
        <p className="text-emerald-400 text-sm font-medium">Purchased ✓</p>
      ) : null}
      ...
      {tx && (
        <a href={getHskExplorerTxUrl(tx)} target="_blank" rel="noopener noreferrer" className="block text-sm text-red-400">
          View transaction ↗
        </a>
      )}
      {state === 'purchased' && (
        <Link href="/purchases" className="block text-sm text-red-400">My Purchases →</Link>
      )}
    </section>
  );
}
