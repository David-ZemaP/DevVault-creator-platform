...
export function PurchaseLibrary({ sales = false }: { sales?: boolean }) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-5 dark:border-zinc-800/80">
        <div>
          <div className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
            <ShoppingBag className="h-3.5 w-3.5 text-zinc-600 dark:text-zinc-200" />
      ...
      {error && (
        <p
          role="alert"
          className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-3 text-xs text-rose-600 dark:text-rose-300"
        >
          {error}
        </p>
      )}
      ...
      {loaded && owner === authenticatedAddress && !visible.length && (
        <div className="rounded-xl border border-zinc-200 bg-white p-10 text-center dark:border-zinc-800/80 dark:bg-zinc-900/30">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">No confirmed {sales ? 'sales' : 'purchases'} yet.</p>
        </div>
      )}

      {sales && visible.length > 0 && (
        <div className="rounded-xl border border-zinc-200 bg-white p-4 space-y-1.5 dark:border-zinc-800/80 dark:bg-zinc-900/40">
          <p className="text-xs sm:text-sm font-medium text-zinc-900 dark:text-white">
          ...
          {Array.from(new Set(visible.map((r) => r.project_id))).map((id) => {
            ...
            return (
              <p key={id} className="text-[11px] text-zinc-500 dark:text-zinc-400">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {visible.map((p) => (
          <article
            key={p.id}
            className="rounded-xl border border-zinc-200 bg-white p-5 space-y-3 shadow-sm transition-all hover:border-zinc-300 dark:border-zinc-800/80 dark:bg-zinc-900/30 dark:shadow-none dark:hover:border-zinc-700"
          >
            {p.publication.coverImage && (
              <img
                src={p.publication.coverImage}
                alt=""
                className="aspect-video w-full rounded-lg object-cover border border-zinc-200 dark:border-zinc-800"
              />
            )}
            <div>
              <h2 className="text-base font-semibold text-zinc-900 dark:text-white">{p.publication.title}</h2>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-zinc-200 dark:border-zinc-800/80">
              <a
                className="inline-flex items-center gap-1 text-xs font-medium text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors"
