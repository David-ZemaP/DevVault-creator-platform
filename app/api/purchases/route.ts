...
export async function GET(request: Request) {
  try {
    ...
    const rows = checked(await commerceDb().from('purchases').select('*, publications(*)').eq(sales ? 'seller_wallet' : 'buyer_wallet', wallet).order('purchased_at', { ascending: false }));
    return privateJson({ purchases: (rows || []).map(row => ({ ...row, publications: undefined, publication: publicPublication(dbRowToPublicationRecord(row.publications)) })) });
