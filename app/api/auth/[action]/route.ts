...
async function issueChallenge(body: Record<string, unknown>) {
  if (typeof body.wallet !== 'string' || !isAddress(body.wallet, { strict: false })) throw new HttpError(400, 'Invalid wallet');
  const chainId = body.chainId === undefined ? AUTH_CHAIN_IDS[0] : Number(body.chainId);
  const challenge = token();
  const wallet = body.wallet.toLowerCase();
  const appOrigin = process.env.APP_ORIGIN || 'http://localhost:3000';
  const row = createWalletChallenge(wallet, chainId, challenge, appOrigin);
...
async function verifyChallenge(body: Record<string, unknown>) {
  ...
  const appOrigin = process.env.APP_ORIGIN || 'http://localhost:3000';
  const wallet = await verifyWalletChallenge(row, challenge, body.signature, appOrigin);
  await revokeSession();
  const session = token();
